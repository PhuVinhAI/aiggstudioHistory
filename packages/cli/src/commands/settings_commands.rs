...
fn get_app_settings_path() -> Result<PathBuf, String> {
    let config_dir = file_cache::get_app_config_dir()?;
    Ok(config_dir.join("app_settings.json"))
}

pub fn get_app_settings() -> Result<models::AppSettings, String> {
...
pub fn set_recent_paths(paths: Vec<String>) -> Result<(), String> {
    let settings_path = get_app_settings_path()?;
    let mut settings = get_app_settings().unwrap_or_default();
...
pub fn update_app_settings(settings: models::AppSettings) -> Result<(), String> {
    let settings_path = get_app_settings_path()?;
    let json_string = serde_json::to_string_pretty(&settings).map_err(|e| e.to_string())?;
    let mut file = File::create(settings_path).map_err(|e| e.to_string())?;
    file.write_all(json_string.as_bytes())
